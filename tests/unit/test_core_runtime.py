...
def test_health_entrypoint_rejects_invalid_not_before() -> None:
    with pytest.raises(SystemExit):
        health_module.main(["--process", "community-worker", "--not-before", "invalid"])
...
@pytest.mark.asyncio
async def test_assignment_deadline_source_returns_bounded_ids() -> None:
    ...
    result = await PostgresAssignmentDeadlineSource(session_factory).due_task_ids(
