...
- После завершения задачи обязательно создать PR в `main`, дождаться обязательных checks и выполнить merge. Deployment допускается только из актуального commit `origin/main`; deploy из рабочей ветки или commit вне `main` запрещён.
...
- Внешние изменения выполняются только при ясном намерении пользователя. Прямое поручение исправить или реализовать задачу разрешает стандартную недеструктивную цепочку Jira → branch → PR → merge после обязательных checks → release → deploy без повторного согласования каждого шага. Остановиться при конфликте, росте риска, failed gate, разрушительном действии или прямой команде владельца. Перед повтором потенциально частично выполненной операции проверить внешнее состояние и исключить дубликат.
...
- Создание, редактирование, комментирование, связывание и переходы задач требуют ясного пользовательского намерения; прямое поручение на реализацию покрывает штатные Jira-действия до `Review`, но не финальный `Done` без production acceptance.
...
- Эксплуатационные и вспомогательные операции по умолчанию реализовывать на Python.
  Исключение — два минимальных root-owned wrapper-а защищённого release:
  `ops/github_deploy_entrypoint.sh` и `ops/deploy_self_hosted.sh`. Новые shell-скрипты
  требуют отдельного архитектурного решения.
