...
- Для любых исправлений Telegram-интерфейса, runtime-поведения бота, карточек,
  callback-кнопок, регистрации, баланса, профилей и пользовательских сценариев
  задача считается готовой только после свежей проверки на рабочем экземпляре
  через живую Telegram-сессию. Нельзя писать «исправлено», «готово» или
  закрывать отчёт, если проверка в живом чате не выполнена и фактический ответ
  бота не подтверждает исправление.
- Перед такой живой Telegram-проверкой обязательно выполнить deployment
  актуального проверенного release на сервер по `docs/operations/PILOT_RUNBOOK.md`.
  Проверка локального кода, CI и опубликованного image без server deploy не
  является проверкой рабочего экземпляра.
