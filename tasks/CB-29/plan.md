...
Без отдельного нового намерения владельца real connector выполняет только
read-only navigation и останавливается **до первой кнопки, которая создаёт
durable-запись**. В текущем интерфейсе это означает:

- не нажимать `Создать приглашение`: действие сразу создаёт приглашение, отдельного
  confirm-шага нет;
- в создании задания остановиться до выбора шаблона: выбор уже создаёт durable draft;
- registration approval/reject, task publish/cancel, assignment settlement,
  dispute, karma и moderation не применяются к чужим production-данным;
- собственная mutation допустима только после отдельного явного поручения,
  заранее указанной disposable сущности и существующего product-level cleanup
  (`revoke`/`cancel`); после cleanup проверяется отсутствие незапланированного
  ledger/domain эффекта;
- если product-level cleanup отсутствует, путь останавливается до первого
  durable-write действия;
- ручной SQL не используется как cleanup, immutable audit/receipts не удаляются.
...
- автоматическое изменение чужих production-заявок, кармы или споров;
...
- **Production mutation:** только собственные тестовые данные либо остановка до
  business decision владельца.
