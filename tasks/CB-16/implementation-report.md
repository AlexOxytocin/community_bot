...
Готовый MVP проверен одним итоговым полным regression после слияния двух обнаруженных ранее
blocking-багов. PostgreSQL 18 regression прошёл `369 passed`, без skip/deselect, с coverage
`80.15%`. Новых дефектов готового функционала не найдено. Свежий operational smoke на реальном
self-hosted runtime подтвердил immutable release, health, backup и isolated restore с нулевым
расхождением ledger/cache.
...
| Пустая и поддерживаемая схема | Пройден | `0009 → 0010`, пустой `head → base → head`, seed и operational constraints |
| Восстановление сохраняет ledger | Пройден | Свежий production backup, isolated restore `0010`, `ledger_mismatch_count=0` |
| Открытых critical/high нет | Пройден | Jira discovery содержит только закрытые CB-20 и CB-21; open blocking JQL пуст |
...
JQL `project = CB AND labels = cb16-regression` возвращает ровно две задачи:
...
Новых дефектов итоговый regression `369 passed` не выявил, поэтому дополнительные Jira Bugs не
создавались.
